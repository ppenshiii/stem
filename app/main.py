...
app = FastAPI(title="SonicSplit Dual API")
...
@app.post("/api/classify_genre")
async def classify_genre(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    if not file.filename:
        raise HTTPException(status_code=400, detail="No file uploaded")
    
    job_id = str(uuid.uuid4())
    ext = os.path.splitext(file.filename)[1] or ".tmp"
    input_path = f"uploads/{job_id}_genre{ext}"
    
    with open(input_path, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)
        
    try:
        # Strictly run lightning fast Genre classification
        ...
        raise HTTPException(status_code=500, detail=f"Genre Pipeline Failure: {str(e)}")
...
async def separate_stems(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    job_id = str(uuid.uuid4())
    ext = os.path.splitext(file.filename)[1] or ".tmp"
    input_path = f"uploads/{job_id}_sep{ext}"
    ...
    try:
        # Strictly run Demucs separation logic safely isolated
        ...
        safe_rmtree(input_path)
        safe_rmtree(out_dir)
        raise HTTPException(status_code=500, detail=f"Separation Pipeline Failure: {str(e)}")
